...
layout: article
read_time: 8
hero_art: /assets/notes/2026-01-27-concept-based-interpretability-in-medical-ai/hero-v1.webp
hero_art_type: image
hero_alt: "A layered representation field resolving into distinct concept structures"
thumbnail: /assets/notes/2026-01-27-concept-based-interpretability-in-medical-ai/cxai.webp
...
social_image: /assets/notes/2026-01-27-concept-based-interpretability-in-medical-ai/social.jpg
...
Concept definition and types of concepts.
...
What is a C-XAI method and how to categorize them.
...
> **Key insight:** Many failures of C-XAI come from a mismatch between *assumed* and *actual* empirically observed representation structure (see also the [reification fallacy](https://surajsrinivas.substack.com/p/the-reification-fallacy-interpretability)).
...
We do not want to just extract concepts, but also assess *functional* relevance:
...
Place C-XAI inside real medical AI systems. Where concepts enter, how they are used, and where they fail in practice.
...
- Modalities and data types
  - Medical imaging (histopathology, radiology, dermatology, ophthalmology)
    - spatial and morphological concepts (tissue structure, shape, ...)
  - Clinical text (reports, notes)
    - semantic and relational concepts (findings, anatomical relations, ...)
  - Signals and measurements (ECG)
    - temporal and frequency-domain concepts (rhythms, ...)
  - Multimodal combinations (image + text + measurements)
    - cross-modal concepts and consistency constraints
- Where concepts attach in the pipeline
  - Input-level concept integration (to constrain learning under limited data)
    - clinically defined features (radiomics, biomarkers, pathology features, ...)
    - knowledge-guided inputs (ontology-aligned attributes, medical knowledge graphs, ...)
  - Representation-level concepts (in latent spaces)
    - concept vectors/subspaces in CNN/ViT embeddings
    - prototypes corresponding to representative patches/regions
  - Aggregation-level concepts (concept emergence)
    - patch-level concepts aggregated into slide-level decisions in histopathology
    - relevant for MIL systems
  - Decision and reasoning-level concepts (explicit usage of concepts to form final prediction)
    - CBMs predicting clinical attributes before diagnosis
    - neuro-symbolic models applying rules over predicted concepts
- What works in practice
  - concept alignment with clinical abstraction (morphology, findings)
  - hybrid supervision (limited expert labels + unsupervised discovery)
  - prototype-based explanations in imaging domains
  - concepts attached at aggregation/decision levels, not individual neurons
- Medical-specific failure modes
  - Data sparsity (limited annotations, inconsistent across experts)
  - Weak supervision (labels at coarse levels - e.g., slides, while concepts are fine-grained)
  - Modality mismatch
  - False semantic alignment (concepts appear plausible to humans but are not causally used by the model)
...
- *Concept representations* for ViT ([SAE](https://arxiv.org/abs/2407.10785)) and MIL ([Concept MIL](https://arxiv.org/abs/2501.02922))
